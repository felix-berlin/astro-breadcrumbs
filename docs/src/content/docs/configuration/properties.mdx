...
The property is validated and should follow this scheme:

```
^[\p{Alphabetic}\p{Numeric}].*[\p{Alphabetic}\p{Numeric}]$
```

_This regular expression matches any string that starts and ends with an alphanumeric character, including both Latin and non-Latin alphabets and numbers._

If customBaseUrl is not valid, a warning appears and customBaseUrl is not used.
