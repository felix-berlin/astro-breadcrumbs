...
describe('Breadcrumbs', () => {
  ...
  it('keeps default styles when cssClasses are provided without disabling', async () => {
    const render = await renderBreadcrumbs(
      { cssClasses: { container: 'custom-nav' } },
      {},
    )
    expect(render).toContain('class="c-breadcrumbs custom-nav"')
    expect(render).toContain('c-breadcrumbs__crumbs')
    expect(render).toContain('c-breadcrumbs__crumb')
  })

  it('renders without default styles when disableDefaultClasses is true', async () => {
    const render = await renderBreadcrumbs(
      { cssClasses: { disableDefaultClasses: true } },
      {},
    )
  ...
  it('renders with custom classes when provided via cssClasses', async () => {
    ...
    expect(render).toContain('c-breadcrumbs custom-nav')
    expect(render).toContain('custom-list')
    expect(render).toContain('custom-item')
    expect(render).toContain('custom-link')
    expect(render).toContain('custom-separator')
  })
})
